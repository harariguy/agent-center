# Stage 1: build the web UI into agent_notify/static/ (vite's outDir).
...
# vite writes to ../agent_notify/static — create the parent it expects.
RUN mkdir -p agent_notify && cd frontend && pnpm build
...
COPY agent_notify/ agent_notify/
COPY --from=frontend /build/agent_notify/static/ agent_notify/static/
RUN pip install --no-cache-dir ".[postgres]"

# State lives here: SQLite database unless DATABASE_URL points elsewhere.
ENV AGENT_NOTIFY_HOME=/data
VOLUME /data
