"""CLI: serve Agent Center, register agents, manage viewer tokens.

    agent-center serve
    agent-center agent add <name>
    agent-center agent list
    agent-center viewer add <label>
    agent-center viewer list
    agent-center viewer revoke <label>
    agent-center prune
"""
...
def _serve(args) -> int:
    ...
    if bind_error := _probe_bind(settings.host, settings.port):
        ...
        return 1
    print(f"Agent Center → http://{settings.host}:{settings.port}", flush=True)
...
def _agent_list(_args) -> int:
    ...
    with make_session_factory(engine)() as db:
        ...
        if not agents:
            print("no agents registered — run: agent-center agent add <name>")
            return 0
        for a in agents:
...
def _viewer_list(_args) -> int:
    ...
    with _viewer_db()() as db:
        ...
        if not tokens:
            print("no viewer tokens — run: agent-center viewer add <label>")
            return 0
        for t in tokens:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="agent-center",
